...
| **SEC-12** | Classique | Fuite via logs | vérifier qu'aucun secret/PII n'apparaît dans logs/traces | logs propres | P1 | M4 | 🟡 **volet logs fait et testé** (ticket 23, `test_sec12_log_leaks.py`) : ni question d'utilisateur, ni contenu de document, ni clé de fournisseur n'atteint une ligne de journal — les arguments d'outil sont tracés par leurs clés, jamais par leurs valeurs. L'**injection de logs** est couverte : un `X-Request-ID` client est validé contre `^[A-Za-z0-9_-]{1,64}$`, ce qui interdit de fabriquer une fausse ligne par saut de ligne, ou de gonfler chaque ligne d'une requête. Vérifié par mutation — désactiver la validation fait rougir neuf tests. **Reste le volet traces**, qui suppose que des traces existent : ticket 24 (Langfuse). |
