...
| **SEC-10** | LLM10 | Unbounded consumption | requêtes très longues, boucles d'agent, flood | rate limit + plafonds tokens/itérations déclenchés | P1 | M2/M4 | 🟡 seul le **plafond de longueur de question** est posé (2 000 caractères, testé). Ni limitation de débit, ni plafond de jetons, ni plafond d'itérations d'agent — M2 et M4. |
| **SEC-11** | Classique | Gestion d'erreurs / XSS | entrées malformées, payload `<script>` dans un champ renvoyé | 4xx propre, sortie échappée, pas de stack trace | P0 | M2 | ✅ `tests/security/test_sec11_error_handling.py` — 7 formes d'entrée malformée en 422, pannes de dépendance en 503 générique, et vérification qu'aucune réponse ne contient de trace, de chemin, de nom de module ni d'hôte interne |
