...
| **SEC-10** | LLM10 | Unbounded consumption | requêtes très longues, boucles d'agent, flood | rate limit + plafonds tokens/itérations déclenchés | P1 | M2/M4 | 🟡 deux plafonds posés et testés (`test_sec10_output_limits.py`) : **longueur de question** (2 000 car.) et **longueur de réponse** (8 000 car., appliqué côté serveur fragment par fragment en streaming). Restent la limitation de débit et le plafond d'itérations d'agent — M2 et M3/M4. |
